...
class BA_StatsViewController: UIViewController {
	...
	private let detailPageApi = BA_Server.Stats_Detail
	...
	*/
	...
	func openWebView(league: BA_Leagues) {
		
		if let baNavController =  self.tabBarController?.navigationController as? BA_NavigationController {
			
			let leagueDetailUrl = String(format: detailPageApi, league.leagueId)
            baNavController.league = league
